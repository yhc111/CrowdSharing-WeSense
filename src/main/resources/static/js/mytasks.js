...
function getDataRow(h){
	...
	var Taskstatuscell = document.createElement("td");//创建第三列
	if(h.taskstatus == 0){
	    Taskstatuscell.innerHTML = "Unfinished";//填充数据
	}else{
        Taskstatuscell.innerHTML = "Finished";//填充数据
	}
	row.appendChild(Taskstatuscell);//加入行中
	
	var Coinscell = document.createElement("td");//创建第五列
	Coinscell.innerHTML = h.coin;//填充数据
	row.appendChild(Coinscell);//加入行中

	var Deadlinecell = document.createElement("td");//创建第四列
	Deadlinecell.innerHTML = h.deadline.substr(0,10);//填充数据
	row.appendChild(Deadlinecell);//加入行中
	...
	var Datacell = document.createElement("td");//创建第七列
